...
#
# ** using only the 1st ensemble member of each model.
#
#
...
ext=".cdf"
folder_tmp="/tmp/tmp_"
tmp="/tmp/cmip5_Var_ensemble_1member"
file_out="tas_ymonvar_1member$ext"
# ----------------------------------------------------------------------
...
### Loop through "*r1i1p1*.nc" in $folder_in 
### containing only the first ensemble members of each model
#
## initialize counter to keep track of variables
#cnt=0;
#
#for i in *r1i1p1*.nc; do 
#
#  # temporary files
#  file_in="$i"
#  file="$folder_tmp$i"
...
## 3) Ensemble of the members / clean up NetCDF file
...
echo_eval "$cmd"
#my_ncatted tas_ymonvar "'multi-year monthly variance of tas'" "'[K^2]'" tas_ymonvar_1member

# delete obsolete global attributes
cmd="ncatted -O -a history,tas_ymonvar,d,,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a associated_files,tas_ymonvar,d,,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a CDI,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a Conventions,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a institution,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a institute_id,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a model_id,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a branch_time,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a contact,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a references,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a initialization_method,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a physics_version,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a frequency,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a creation_date,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a tracking_id,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a version_number,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a product,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a table_id,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a title,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a modeling_realm,global,d,, $file_out"
echo_eval "$cmd"
cmd="ncatted -O -a cmor_version,global,d,, $file_out"
echo_eval "$cmd"
