...
% plot_2panels.m
%
% plotting procedure for 2 -miller-map figure, outputted as an
% .eps or .png
%
...
for i=1:Npanel
  ...
  % eval data_shift to get `z$i' and `x$i'
  cmd = ['[z',code,',x',code,']=' ...
...
cbar_pos = [0.86,0.25,0.03,0.5,0.07];     % cbar l, b, w, h, tick length 
cbar_offset = 0.05;                      % cbar horiz. offset 
annotate_pos = [-3.05,-0.8];              % on-plot annot. positions
output_size = [20,20];                   % output size in cm
% ----------------------------------------------------------------------
...
%% Looping through the panels of the 2 x 1 figure
...
splot_correc = [0,0.5*cbar_offset,0,0];
